...
import com.pathvu.accesspath2020.exception.InitializationException
...
class PlaceAPI private constructor(
    ...
    private fun parseDetailsData(jsonResults: StringBuilder, listener: OnPlacesDetailsListener) {
        ...
        val id = if(resultJsonObject.isNull(ID)) "" else resultJsonObject.getString(ID)
...
//    val globalPlusCode = plusCode.getString(GLOBAL_CODE)
        ...
        listener.onPlaceDetailsFetched(
            PlaceDetails(
                id, resultJsonObject.getString(NAME), address, lat,
                lng, placeId, url, utcOffset, vicinity, 0.toFloat()
            )
        )
    }
